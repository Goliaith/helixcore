...
def ensure_lightweight_active_session(task_slug: str, initial_focus: str = "") -> None:
    ...
    if task_slug not in state.get("active_sessions", {}):
        ...
        state.setdefault("active_sessions", {})[task_slug] = {"safety_id": f"lightweight-{task_slug[:20]}", "started_at": now, "last_heartbeat": now, "current_focus": initial_focus or "Ad-hoc session", "key_decisions": [], "phase_handoffs": [], "checkpoints": [], "automation_results": {}, "related_sessions": [], "_lightweight": True, "initiative_mode": None}
...
def record_phase_handoff(summary: str, next_focus: str = "", task_slug: Optional[str] = None) -> None:
    ...
    if not slug or slug not in state.get("active_sessions", {}): return
...
def get_initiative_status(task_slug: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {"task_slug": slug, "initiative_mode": mode, "is_disciplined_initiative": mode == "disciplined", "activated_at": live.get("initiative_mode_activated_at")}

def set_initiative_mode(task_slug: str, mode: str = "disciplined", source: str = "explicit") -> bool:
    if not task_slug: return False
    try:
        state = _load_orchestration_state()
        if task_slug not in state.get("active_sessions", {}):
            ensure_lightweight_active_session(task_slug)
        session = state["active_sessions"][task_slug]
        if mode is None or mode == "":
            session.pop("initiative_mode", None)
        else:
            now = datetime.now(timezone.utc).isoformat()
            session["initiative_mode"] = mode
            session["initiative_mode_activated_at"] = now
            session["initiative_mode_source"] = source
        _save_orchestration_state(state)
        try:
            if mode == "disciplined":
                _persist_decision(task_slug, f"Activated Disciplined Initiative Mode (source: {source})", category="initiative_mode")
            else:
                _persist_decision(task_slug, "Deactivated Disciplined Initiative Mode", category="initiative_mode")
        except Exception:
            pass
        return True
    except Exception:
...
def is_disciplined_initiative(task_slug: Optional[str]) -> bool:
    if not task_slug: return False
    if os.environ.get("GROK_DISCIPLINED_INITIATIVE_FORCE") == "1": return True
    try:
        state = _load_orchestration_state()
        session = state.get("active_sessions", {}).get(task_slug, {})
        return session.get("initiative_mode") == "disciplined"
    except Exception:
        return False

# Stubs for the rest of the API surface used by __init__ / golden paths
